...
Reading progress is stored two ways. `Book.currentPage` is the bookmark. `ReadingSession` records pages per book per day and drives the heatmap, streaks and monthly totals. Moving the slider, typing a page or pressing the arrows beside the page number logs the difference as today's session. An arrow steps one page, and holding it keeps stepping. Every save waits until the changes stop, so a run of steps is logged once. The day after the page number ("today") picks the day a save is logged on, for reading done on a day you did not update: yesterday, a few days back, or any past date. It applies to the next save, then goes back to today. Clicking a day on the heatmap opens that day's books, being read or finished around then, to fill in or fix the pages read. A book in progress moves its page by the difference, and a finished book only takes pages that were never logged. Reading dated before a book's start moves the start back. Both use `lib/progress.ts`. The first save of a book at page 0 is where you already were, not a day's reading: it sets `Book.trackedFrom` and logs nothing, so those pages count in all-time totals only. From a page set in the form, that page is the start and the rest is logged. A step back takes pages off the log, latest day first, only as far as the new page needs. Moving a book back to Want to read deletes its reading days. The dashboard's all-time pages and the analytics page's All time use the same count. The rules are in `lib/progress.ts`, `lib/stats.ts` and `lib/analytics.ts`.
...
Both 3D views stand the books on a shelf board seen from a little above: a top surface running back to the wall, a lit lip, the front face and a soft shadow on the wall. It is drawn from theme tokens (`--shelf-*` in `app/globals.css`), so it reads the same way in light and dark. In Spines the board's lines are projected with the books' own perspective, so books stand on it and not over its edge. The camera sits over the middle of each row, so the board's ends slant in toward the wall, and each row leaves some empty board beside its first and last book. Display gets the same ends: it measures its grid to know how many rows there are and gives each row its own board.

The **Spines** view stands the books at an angle, pressed one against the next, so each shows its spine and a strip of cover. Pointing at a book slides it out along its own cover, which never touches its neighbors, and then either turns it to face you or leaves it at its angle. **Adjust** sets that choice, the angle (40° to 75°, default 62°) and the space between books (0 to 80 px, default 14 px). The books lean in parallel, so that space is also how much of each cover shows. The settings stay in this browser. Only one book turns at a time: when the pointer moves on, the first book goes back before the next one turns. On a phone the first tap pulls a book out and the second opens it. A spine's thickness follows the page count, one pixel per 16 pages, so a book twice as long is twice as thick. Next to the paperback's height that is close to real paper. Books under 192 pages share the thinnest spine so the title still fits, a count above 1500 is treated as 1500 so a typo cannot fill a row, and a book with no page count gets a typical 320. A thick book swings its back corner behind it as it turns, so it slides further out first, as far as it needs to clear both neighbors. Books with a cover image take their spine color from the image's left edge, with the title in cream or near-black, whichever stands out more. The books share one 3D scene, so the browser draws whichever is in front. The geometry is in `lib/leaning-shelf.ts`, and its tests check that no book passes through another at any angle, strip width, page count or hand-off.
...
Goodreads now and then answers a good link with an error or a page that has no book in it, and the same link works a moment later. So a Goodreads link is tried three times, a second or two apart, and a failed page is never kept, so trying again fetches it fresh. If it still fails, Add many says what Goodreads sent. When nothing else on a page can be read, its title, such as "Iron Gold (Red Rising Saga, #4) by Pierce Brown | Goodreads", still gives the title, author and series. Pasted links are read from www.goodreads.com whatever form they come in, such as goodreads.com or m.goodreads.com.
